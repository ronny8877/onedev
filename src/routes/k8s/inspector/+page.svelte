...
spec:
  ...
  template:
	...
	const k8sResourceTypes: Record<string, { color: string; requiredFields: string[] }> = {
		Pod: {
			color: 'success',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.containers']
		},
		Deployment: {
			color: 'primary',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.selector', 'spec.template']
		},
		Service: {
			color: 'info',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.ports']
		},
		...
		Ingress: {
			color: 'accent',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.rules']
		},
		StatefulSet: {
			color: 'primary',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.serviceName', 'spec.template']
		},
		DaemonSet: {
			color: 'secondary',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.selector', 'spec.template']
		...
		Job: {
			color: 'info',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.template']
		},
		CronJob: {
			color: 'warning',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.schedule', 'spec.jobTemplate']
		},
		PersistentVolumeClaim: {
			color: 'accent',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.accessModes', 'spec.resources']
		},
		PersistentVolume: {
			color: 'secondary',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.capacity', 'spec.accessModes']
		},
		...
		RoleBinding: {
			color: 'error',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'roleRef', 'subjects']
		},
		ClusterRole: {
			color: 'primary',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'rules']
		},
		ClusterRoleBinding: {
			color: 'secondary',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'roleRef', 'subjects']
		...
		HorizontalPodAutoscaler: {
			color: 'info',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.scaleTargetRef']
		},
		NetworkPolicy: {
			color: 'warning',
			requiredFields: ['apiVersion', 'kind', 'metadata.name', 'spec.podSelector']
		}
	};
		...
		try {
			...
			if (!isK8s) {
				if (!kind)
					issues.push({
						type: 'warning',
						message: 'No "kind" field. This may not be a K8s manifest.'
					});
				if (!apiVersion)
					issues.push({
						type: 'warning',
						message: 'No "apiVersion" field. This may not be a K8s manifest.'
					});
			...
			if (apiVersion?.includes('v1beta1') && kind === 'Ingress') {
				issues.push({
					type: 'warning',
					message: 'Ingress v1beta1 deprecated. Use networking.k8s.io/v1.'
				});
			}

			// Check for container specs in workloads
			const containers =
				(getNestedValue(parsed, 'spec.template.spec.containers') as unknown[] | undefined) ||
			...
			if (containers && Array.isArray(containers)) {
				containers.forEach((container, i) => {
					...
					if (!c.image) {
						issues.push({
							type: 'error',
							message: `Container "${c.name || i + 1}" missing "image"`
						});
					}
					if (!c.resources) {
						issues.push({
							type: 'warning',
							message: `Container "${c.name}" has no resource limits`
						});
					}
				});
			}
...
<ToolWrapper>
	<div class="flex flex-col gap-6">
		...
		<div class="grid gap-6 lg:grid-cols-2">
			...
			<div class="card h-fit rounded-2xl bg-base-200">
				<div class="card-body p-4">
					...
					<textarea
						...
						class="textarea-bordered textarea min-h-80 w-full resize-y font-mono text-sm leading-relaxed"
			...
			<div class="space-y-4">
				...
				{:else if result.isK8s}
						...
						<div class="card-body p-4">
							<div class="mb-4 flex items-center justify-between">
								<div>
									<div class="flex items-center gap-2">
										...
										<span class="badge badge-{result.resourceInfo?.color || 'ghost'} badge-sm"
											>{result.apiVersion}</span
										>
									</div>
									<p class="font-mono text-sm text-base-content/70">{result.name}</p>
								</div>
								{#if result.isValid}
							...
							<div class="grid gap-3 sm:grid-cols-2">
								<div class="rounded-lg bg-base-300/50 p-3">
									<p class="mb-1 text-xs text-base-content/60">Name</p>
									<p class="truncate font-mono text-sm font-bold">{result.name || '—'}</p>
								</div>
								<div class="rounded-lg bg-base-300/50 p-3">
									<p class="mb-1 text-xs text-base-content/60">Namespace</p>
							...
							{#if result.labels && Object.keys(result.labels).length > 0}
								<div class="mt-3">
									<p class="mb-2 text-xs text-base-content/60">Labels</p>
									<div class="flex flex-wrap gap-1">
										{#each Object.entries(result.labels) as [key, value]}
											<span class="badge badge-ghost font-mono badge-sm">{key}={value}</span>
										{/each}
									</div>
								</div>
							{/if}
									...
									<div class="space-y-1">
										{#each Object.entries(result.summary) as [key, value]}
											<div class="flex justify-between text-sm">
												...
												<span class="max-w-48 truncate font-mono font-medium">{value}</span>
											</div>
										{/each}
									</div>
								</div>
						...
						<div class="card rounded-xl bg-base-200">
							<div class="card-body p-4">
								<div class="mb-3 flex items-center justify-between">
									<h4 class="text-sm font-bold">Validation</h4>
								...
								<div class="max-h-40 space-y-1 overflow-y-auto">
									{#each result.issues as issue}
										<div
											class="flex items-center gap-2 rounded p-2 text-sm {issue.type === 'error'
												? 'bg-error/10'
												: 'bg-warning/10'}"
										>
					...
					{:else}
						<div class="card rounded-xl border border-success/20 bg-success/10">
				...
				{:else if input.trim() && !result.isK8s}
					<div class="card rounded-xl border border-warning/20 bg-warning/10">
						<div class="card-body p-4">
							<h4 class="mb-2 font-bold text-warning">Not a K8s Manifest</h4>
							<p class="text-sm text-base-content/70">
								K8s manifests require <code>apiVersion</code> and <code>kind</code> fields.
							</p>
						</div>
					</div>
				{:else}
		...
		<div class="card rounded-xl bg-base-200">
			<div class="card-body p-4">
				<h4 class="mb-2 text-sm font-semibold">Supported Resources</h4>
