<script lang="ts">
	...
	let uppercase = $state(true); // CRC32 is commonly shown uppercase
	...
	const ALGORITHM = 'CRC32' as const;
...
>
	<div class="flex flex-col gap-6">
		...
		{#if inputMode === 'text'}
			<textarea
				bind:value={input}
				placeholder="Type or paste text to calculate CRC32..."
		...
		{#if inputMode === 'file'}
			...
			>
				...
				{#if progress && isProcessing}
					<div class="px-4 pb-4">
						<progress class="progress progress-neutral w-full" value={progress.percent} max="100"></progress>
					</div>
				{/if}
			</div>
		{/if}
				...
				<div class="card-body p-4">
					<div class="flex items-center justify-between gap-2 mb-3">
						<div class="flex items-center gap-2">
							<span class="font-semibold">CRC32 Checksum</span>
							<span class="badge badge-neutral badge-sm">32 bits</span>
							<span class="text-xs text-base-content/50">{formatTime(result.time)}</span>
						</div>
						<CopyButton text={formatHash(result.hex)} size="sm" />
					</div>
					<code class="block font-mono text-2xl text-center break-all bg-base-300/50 p-4 rounded-lg select-all tracking-wider">
						{formatHash(result.hex)}
					</code>
				</div>
			</div>
		{/if}
		...
		{#if isProcessing && inputMode === 'text'}
			<div class="flex items-center justify-center gap-2 py-4">
				...
				<span class="text-sm text-base-content/70">Calculating CRC32...</span>
			</div>
		{/if}
		...
		<div class="card bg-base-200 rounded-xl">
			<div class="card-body py-4">
				<h4 class="text-sm font-semibold">About CRC32</h4>
				<ul class="mt-2 space-y-1 text-sm text-base-content/70">
					<li>• <strong>Output:</strong> 32-bit (8 hex characters)</li>
					<li>• <strong>Speed:</strong> Extremely fast, designed for error detection</li>
					<li>• <strong>Security:</strong> NOT cryptographic, easy to forge</li>
					<li>• <strong>Use cases:</strong> ZIP files, network protocols, error detection, quick comparisons</li>
				</ul>
			</div>
		</div>
	</div>
