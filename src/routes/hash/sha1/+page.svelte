<script lang="ts">
	...
	const content = hashToolsContent['sha1'];
	...
	const ALGORITHM = 'SHA-1' as const;
	...
	<div class="flex flex-col gap-6">
		...
		{#if inputMode === 'file'}
			...
			>
				...
				{#if progress && isProcessing}
					<div class="px-4 pb-4">
						<progress class="progress progress-info w-full" value={progress.percent} max="100"></progress>
					</div>
				{/if}
			</div>
		{/if}
				...
				<div class="card-body p-4">
					<div class="flex items-center justify-between gap-2 mb-3">
						<div class="flex items-center gap-2">
							<span class="font-semibold">SHA-1 Hash</span>
							<span class="badge badge-info badge-sm">160 bits</span>
		...
		{#if isProcessing && inputMode === 'text'}
			<div class="flex items-center justify-center gap-2 py-4">
				...
				<span class="text-sm text-base-content/70">Generating SHA-1...</span>
			</div>
		{/if}
		...
		<div class="alert alert-warning rounded-xl">
			...
			<span><strong>Legacy algorithm:</strong> SHA-1 is deprecated for security. Use SHA-256 for new projects.</span>
		</div>

		<!-- Info -->
		<div class="card bg-base-200 rounded-xl">
			<div class="card-body py-4">
				<h4 class="text-sm font-semibold">About SHA-1</h4>
				<ul class="mt-2 space-y-1 text-sm text-base-content/70">
					<li>• <strong>Output:</strong> 160-bit (40 hex characters)</li>
					<li>• <strong>Security:</strong> Collision attacks demonstrated (SHAttered, 2017)</li>
					<li>• <strong>Still used in:</strong> Git commit hashes, legacy certificate validation</li>
					<li>• <strong>Recommendation:</strong> Migrate to SHA-256 for security-sensitive use</li>
				</ul>
			</div>
		</div>
		<!-- Content Sections -->
