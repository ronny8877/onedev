	...
	<div class="flex flex-col gap-6">
		...
		{#if inputMode === 'file'}
			...
			>
				...
				{#if progress && isProcessing}
					<div class="px-4 pb-4">
						<progress class="progress progress-success w-full" value={progress.percent} max="100"></progress>
					</div>
				{/if}
			</div>
		{/if}
				...
				<div class="card-body p-4">
					<div class="flex items-center justify-between gap-2 mb-3">
						<div class="flex items-center gap-2">
							<span class="font-semibold">SHA-256 Hash</span>
							<span class="badge badge-success badge-sm">256 bits</span>
		...
		{#if isProcessing && inputMode === 'text'}
			<div class="flex items-center justify-center gap-2 py-4">
				...
				<span class="text-sm text-base-content/70">Generating SHA-256...</span>
			</div>
		{/if}
		...
		<div class="card bg-base-200 rounded-xl">
			<div class="card-body py-4">
				<h4 class="text-sm font-semibold">About SHA-256</h4>
				<ul class="mt-2 space-y-1 text-sm text-base-content/70">
					<li>• <strong>Output:</strong> 256-bit (64 hex characters)</li>
					<li>• <strong>Security:</strong> Industry standard, no known practical attacks</li>
					<li>• <strong>Used in:</strong> Bitcoin, TLS/SSL certificates, file verification, digital signatures</li>
					<li>• <strong>Part of:</strong> SHA-2 family (SHA-224, SHA-256, SHA-384, SHA-512)</li>
				</ul>
			</div>
		</div>
	</div>
