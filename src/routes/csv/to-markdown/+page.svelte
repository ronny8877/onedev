<script lang="ts">
	...
	import { csvToMarkdown, SAMPLE_CSV } from '$lib/utils/csv';
	import { csvToolsContent } from '$lib/config/content/csv-tools-content';

	const content = csvToolsContent['to-markdown'];
	let input = $state('');
	let result = $derived(input.trim() ? csvToMarkdown(input) : { ok: true, output: '', error: null });
</script>
...
<ToolWrapper lastUpdated="2026-08-20">
	<div class="flex flex-col gap-6">
		...
		<div class="grid gap-6 lg:grid-cols-2">
			...
			<div class="card bg-base-200 rounded-2xl"><div class="card-body p-4">
				<h3 class="font-bold mb-3">Markdown</h3>
