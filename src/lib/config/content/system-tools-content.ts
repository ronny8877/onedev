...
type SystemToolContent = ToolContent;
...
export const systemToolsContent: Record<string, SystemToolContent> = {
	info: {
		...
		examples: [
			{
				label: 'Checking GPU for WebGL games',
				code: 'GPU Renderer: NVIDIA GeForce RTX 3080\nWebGL Version: 2.0\nMax Texture Size: 16384px',
				isValid: true
			},
			{
				label: 'Verifying memory for heavy apps',
				code: 'Device Memory: ≥8 GB\nLogical CPU Cores: 12\nNote: Browser caps at 8GB',
				isValid: true
			},
			{
				label: 'Display info for responsive design',
				code: 'Screen Resolution: 2560 × 1440\nDevice Pixel Ratio: 2x\nActual Pixels: 5120 × 2880',
				isValid: true
			}
		],
		faqs: [
			{
				question: 'Why does memory show "≥8 GB" instead of exact amount?',
				answer:
					'<p>Browsers cap the Device Memory API at 8GB to prevent fingerprinting and protect user privacy. If your device has 16GB+ RAM, the browser will only report 8GB.</p>'
			},
			{
				question: 'Is my GPU information exposed to websites?',
				answer:
					'<p>By default, browsers hide GPU details. This tool uses the WebGL debug renderer extension to show your GPU, but this only works if your browser allows it. Many browsers now block this for privacy.</p>'
			},
			{
				question: 'Why is architecture "indeterminate" on my Mac?',
				answer:
					'<p>Modern Macs report "MacIntel" even on Apple Silicon chips due to Rosetta compatibility and browser privacy measures. The browser intentionally obscures whether you\'re using Intel or ARM.</p>'
			},
			{
				question: 'Can I trust the battery information?',
				answer:
					"<p>Battery data is approximate and rounded by the browser. It's useful for debugging but not precise enough for critical applications. Some browsers (like Firefox) have disabled the Battery API entirely.</p>"
			},
			{
				question: 'How can I copy all this data for a bug report?',
				answer:
					'<p>Click the "Copy as JSON" button to copy all system information in JSON format. You can paste this into bug reports or support tickets for developers to analyze.</p>'
			}
		],
		relatedTools: [
			{
				name: 'Browser Info',
				path: '/system/browser',
				description: 'Detect browser name, version, and capabilities'
			},
			{
				name: 'Network Info',
				path: '/system/network',
				description: 'Check connection type and public IP'
			},
			{
				name: 'Media Devices',
				path: '/system/media',
				description: 'List cameras, mics, and speakers'
			},
			{
				name: 'Permissions',
				path: '/system/permissions',
				description: 'Check browser permission states'
			}
		],
		tips: [
	...
	browser: {
		...
		examples: [
			{
				label: 'Checking WebAssembly support',
				code: 'WebAssembly: Yes\nSharedArrayBuffer: Yes\nWeb Workers: Yes',
				isValid: true
			},
			{
				label: 'Verifying PWA prerequisites',
				code: 'Service Worker: Yes\nNotifications: Yes\nLocal Storage: Yes\nIndexedDB: Yes',
				isValid: true
			},
			{
				label: 'Testing emerging APIs',
				code: 'WebGPU: Yes\nWeb Bluetooth: No\nWeb USB: No\nWeb Serial: No',
				isValid: true
			}
		],
		faqs: [
			{
				question: "What's the difference between browser version and engine version?",
				answer:
					'<p>Browser version is the marketing version (e.g., Chrome 120). Engine version refers to the rendering engine like Blink, WebKit, or Gecko. This tool shows the browser version, which is usually more useful for compatibility checks.</p>'
			},
			{
				question: 'Why does "Do Not Track" show "Not set"?',
				answer:
					"<p>Do Not Track (DNT) is a deprecated privacy signal. Most modern browsers don't set it because it was never widely respected by websites. Browsers now use other privacy mechanisms like tracking protection.</p>"
			},
			{
				question: 'Can I use this for browser fingerprinting?',
				answer:
					'<p>While this tool shows browser capabilities, using it for fingerprinting is discouraged. Modern browsers actively fight fingerprinting by standardizing or hiding unique identifiers. Use feature detection for legitimate compatibility checks only.</p>'
			},
			{
				question: 'What does "Cookies: Disabled" mean?',
				answer:
					'<p>If cookies are disabled, your browser blocks all website cookies. This breaks most session-based authentication. Third-party cookie blocking (common in privacy-focused browsers) shows as "Enabled" here.</p>'
			},
			{
				question: 'How accurate is the timezone detection?',
				answer:
					'<p>Timezone detection via <code>Intl.DateTimeFormat</code> is very accurate and respects your system settings. It shows the IANA timezone name (e.g., "America/New_York"), which is more precise than UTC offsets.</p>'
			}
		],
		relatedTools: [
			{
				name: 'System Info',
				path: '/system/info',
				description: 'View OS, CPU, memory, and GPU info'
			},
			{
				name: 'Network Info',
				path: '/system/network',
				description: 'Check internet connection details'
			},
			{
				name: 'Permissions',
				path: '/system/permissions',
				description: 'Check granted browser permissions'
			},
			{
				name: 'Media Devices',
				path: '/system/media',
				description: 'Detect cameras and microphones'
			}
		],
		tips: [
	...
	network: {
		...
		examples: [
			{
				label: 'Fast WiFi connection',
				code: 'Type: wifi\nEffective Type: 4g\nDownlink: 10 Mbps\nRTT: 50 ms',
				isValid: true
			},
			{
				label: 'Slow mobile data',
				code: 'Type: cellular\nEffective Type: 2g\nDownlink: 0.4 Mbps\nRTT: 800 ms',
				isValid: true
			},
			{
				label: 'Checking if online',
		...
		faqs: [
			{
				question: 'Why does connection type show "unknown"?',
				answer:
					'<p>Some browsers don\'t support the Network Information API, or your device/OS doesn\'t expose connection type. Desktop computers on Ethernet often show "unknown" for privacy reasons.</p>'
			},
			{
				question: 'Is the downlink speed accurate?',
				answer:
					"<p>Downlink is an <em>estimate</em> based on recent transfers, not a real-time speed test. It's useful for categorization (fast/slow) but not as precise as running a dedicated speed test.</p>"
			},
			{
				question: 'What does "effective type: 2g" mean on WiFi?',
				answer:
					'<p>Effective type reflects actual performance, not connection type. If your WiFi is congested or has high latency, it may be classified as "2g" even though you\'re on WiFi. This helps apps adapt to real conditions.</p>'
			},
			{
				question: 'Can I get my exact ISP from this tool?',
				answer:
					"<p>This tool shows your public IP, which can be used to look up your ISP via third-party services. However, the browser doesn't directly expose ISP information for privacy reasons.</p>"
			},
			{
				question: 'How often does the connection info update?',
				answer:
					'<p>The browser updates connection info periodically based on real network performance. Apps can listen to <code>change</code> events to react when the connection type or speed changes.</p>'
			}
		],
		relatedTools: [
			...
			{ name: 'Browser Info', path: '/system/browser', description: 'Check browser capabilities' },
			{
				name: 'Media Devices',
				path: '/system/media',
				description: 'List available cameras and mics'
			},
	...
	media: {
		...
		examples: [
			{
				label: 'Laptop with built-in camera',
				code: 'Cameras: 1\n- FaceTime HD Camera (Built-in)\nMicrophones: 1\n- Built-in Microphone',
				isValid: true
			},
			{
				label: 'Desktop with external devices',
				code: 'Cameras: 2\n- Logitech C920 (USB)\n- Razer Kiyo (USB)\nMicrophones: 3\n- Blue Yeti (USB)\n- Headset Mic\n- Built-in',
				isValid: true
			},
			{
		...
		faqs: [
			{
				question: 'Why do I see "Camera 1" instead of the actual device name?',
				answer:
					'<p>For privacy, browsers hide device labels until you grant camera/microphone permission. Click "Request Permission" to see real device names.</p>'
			},
			{
				question: 'Can I select which camera to use for video calls?',
				answer:
					'<p>Yes! Apps can use <code>getUserMedia()</code> with a specific <code>deviceId</code> to target a particular camera. This tool shows all available device IDs.</p>'
			},
			{
				question: "Why isn't my USB webcam showing up?",
				answer:
					'<p>Make sure the camera is plugged in and not in use by another app. Try refreshing the page or restarting your browser. On Linux, check that your user has permissions to access <code>/dev/video*</code> devices.</p>'
			},
			{
				question: "What's the difference between audioinput and audiooutput?",
				answer:
					'<p><code>audioinput</code> devices are microphones (record audio). <code>audiooutput</code> devices are speakers/headphones (play audio). Most browsers support input enumeration but not all support output selection.</p>'
			},
			{
				question: 'Can websites access my camera without permission?',
				answer:
					'<p>No. Browsers require explicit user permission before accessing camera or microphone. Permissions are granted per-origin and can be revoked at any time in browser settings.</p>'
			}
		],
		relatedTools: [
			{
				name: 'Permissions',
				path: '/system/permissions',
				description: 'Check camera/mic permission status'
			},
			{ name: 'System Info', path: '/system/info', description: 'View OS and hardware details' },
			{
				name: 'Browser Info',
				path: '/system/browser',
				description: 'Check browser feature support'
			},
	...
	permissions: {
		...
		examples: [
			{
				label: 'Checking camera permission',
				code: "Camera: prompt\n→ User hasn't decided\n→ Can request via getUserMedia()",
				isValid: true
			},
			{
				label: 'Location already granted',
				code: 'Geolocation: granted\n→ Can call navigator.geolocation immediately\n→ No prompt needed',
				isValid: true
			},
			{
				label: 'Notifications blocked',
				code: 'Notifications: denied\n→ User declined notifications\n→ Must change in browser settings',
				isValid: true
			}
		],
		faqs: [
			{
				question: 'What does "prompt" mean for a permission?',
				answer:
					'<p>"Prompt" means the user hasn\'t made a decision yet. The next time your app requests this permission, the browser will show a permission dialog. If the user allows or denies, the state changes to "granted" or "denied".</p>'
			},
			{
				question: 'Can I reset a denied permission from this tool?',
				answer:
					'<p>No. Once a user denies a permission, it can only be reset in browser settings (usually in the address bar lock icon or site settings). This prevents websites from bypassing user decisions.</p>'
			},
			{
				question: "Why can't I query all permissions at once?",
				answer:
					'<p>For privacy, the Permissions API only works for a limited subset of permissions. Sensitive permissions like camera/microphone can only be queried after being requested with <code>getUserMedia()</code>.</p>'
			},
			{
				question: "What's the difference between checking and requesting permissions?",
				answer:
					'<p>Checking (via Permissions API) tells you the current state without prompting. Requesting (via feature API like <code>getUserMedia()</code>) triggers a browser dialog if state is "prompt". Always check first to avoid unnecessary prompts.</p>'
			},
			{
				question: 'Can websites track me through permissions?',
				answer:
					'<p>Permission states can contribute to browser fingerprinting, but modern browsers mitigate this. The Permissions API is designed to improve UX without exposing more information than feature detection already reveals.</p>'
			}
		],
		relatedTools: [
			{
				name: 'Media Devices',
				path: '/system/media',
				description: 'List and test camera/mic devices'
			},
			{ name: 'Browser Info', path: '/system/browser', description: 'Check browser capabilities' },
