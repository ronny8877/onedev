...
export const textToolsContent: Record<string, TextToolContent> = {
	'string-compare': {
		...
		examples: [
			{ label: 'Finding typo differences', code: 'String 1: "The quick brown fox"\nString 2: "The qiuck brown fox"\n→ Highlights "quick" vs "qiuck"', isValid: true },
			{ label: 'Case-insensitive match', code: 'String 1: "Hello World"\nString 2: "hello world"\n→ Match (case-insensitive mode)', isValid: true },
			{ label: 'API response comparison', code: 'Response 1: {"status": "success"}\nResponse 2: {"status": "failure"}\n→ Highlights "success" vs "failure"', isValid: true }
		],
		faqs: [
			{ question: 'What\'s the difference between character and word comparison?', answer: '<p>Character comparison checks every single character, making it ideal for catching typos. Word comparison treats each word as a unit, which is better for comparing sentences or paragraphs where word order matters.</p>' },
			{ question: 'Does this tool support regex patterns?', answer: '<p>No, this is a direct string comparison tool. For pattern-based search and comparison, use the Find & Replace tool or Text Diff tool which support regex.</p>' },
			{ question: 'Can I compare multi-line strings?', answer: '<p>Yes! The tool handles multi-line strings and will show differences line by line, similar to a code diff viewer.</p>' },
			{ question: 'What does "case-insensitive" mean?', answer: '<p>In case-insensitive mode, uppercase and lowercase letters are treated as equal. For example, "Hello", "HELLO", and "hello" would all be considered identical.</p>' },
			{ question: 'How can I use this for debugging?', answer: '<p>Paste expected output vs actual output to quickly spot differences. This is especially useful for debugging string manipulation, API responses, or text transformations.</p>' }
		],
		relatedTools: [
			{ name: 'Text Diff', path: '/text/diff', description: 'Line-by-line text comparison' },
			{ name: 'Find & Replace', path: '/text/find-replace', description: 'Find and replace with regex' },
			{ name: 'Remove Duplicates', path: '/text/remove-duplicates', description: 'Remove duplicate lines' },
			{ name: 'Case Converter', path: '/text/case-converter', description: 'Convert text case formats' }
		],
		tips: [
	...
	'case-converter': {
		...
		examples: [
			{ label: 'JavaScript variable to Python', code: 'Input: myVariableName\ncamelCase → snake_case\nOutput: my_variable_name', isValid: true },
			{ label: 'Creating URL slug', code: 'Input: My Blog Post Title\nTitle Case → kebab-case\nOutput: my-blog-post-title', isValid: true },
			{ label: 'Database constant', code: 'Input: max retry count\nlowercase → UPPER_SNAKE_CASE\nOutput: MAX_RETRY_COUNT', isValid: true }
		],
		faqs: [
			{ question: 'What\'s the difference between camelCase and PascalCase?', answer: '<p>camelCase starts with a lowercase letter (myVariable), while PascalCase starts with uppercase (MyClass). Both capitalize subsequent words.</p>' },
			{ question: 'When should I use snake_case vs kebab-case?', answer: '<p>Use snake_case for code (Python, Ruby, database columns) because underscores are valid in identifiers. Use kebab-case for URLs and file names because hyphens are more readable in URLs.</p>' },
			{ question: 'Can this tool handle special characters?', answer: '<p>Yes! The tool can preserve or remove special characters like @, #, $, etc. depending on your needs. Non-alphabetic characters are typically removed in programming identifiers.</p>' },
			{ question: 'What is SCREAMING_SNAKE_CASE used for?', answer: '<p>SCREAMING_SNAKE_CASE (all uppercase with underscores) is conventionally used for constants in many programming languages, like MAX_BUFFER_SIZE or API_KEY.</p>' },
			{ question: 'Does this preserve acronyms correctly?', answer: '<p>The tool tries to preserve acronyms intelligently. For example, "HTTPSConnection" → "https_connection" in snake_case, keeping the acronym together.</p>' }
		],
		relatedTools: [
			{ name: 'String Compare', path: '/text/string-compare', description: 'Compare text differences' },
			{ name: 'Find & Replace', path: '/text/find-replace', description: 'Find and replace patterns' },
			{ name: 'Remove Duplicates', path: '/text/remove-duplicates', description: 'Remove duplicate text' },
	...
	'line-tools': {
		...
		examples: [
			{ label: 'Sorting import statements', code: 'Input:\nimport React\nimport axios\nimport lodash\n→ Sorted alphabetically', isValid: true },
			{ label: 'Removing duplicates', code: 'Input:\napple\nbanana\napple\ncherry\n→ Output: apple, banana, cherry', isValid: true },
			{ label: 'Cleaning whitespace', code: 'Input:\n  hello  \n world  \n→ Output: hello, world (trimmed)', isValid: true }
		],
		faqs: [
			{ question: 'What\'s the difference between normal sort and natural sort?', answer: '<p>Normal sort treats numbers as strings, so "10" comes before "2". Natural sort recognizes numbers, so "2" comes before "10". Use natural sort for lists with numbers.</p>' },
			{ question: 'Does removing duplicates preserve order?', answer: '<p>Yes! The tool keeps the first occurrence of each unique line in its original position, removing only the subsequent duplicates.</p>' },
			{ question: 'Can I sort case-insensitively?', answer: '<p>Yes! Case-insensitive sort treats "Apple" and "apple" as the same for sorting purposes, but preserves the original case in the output.</p>' },
			{ question: 'What happens to empty lines when sorting?', answer: '<p>Empty lines are typically moved to the beginning or end of the sorted list. You can also choose to remove them entirely before sorting.</p>' },
			{ question: 'Can I number lines like a code editor?', answer: '<p>Yes! The line numbering feature adds sequential numbers to the beginning of each line, useful for creating numbered lists or referencing specific lines.</p>' }
		],
		relatedTools: [
			{ name: 'Remove Duplicates', path: '/text/remove-duplicates', description: 'Dedicated duplicate removal' },
			{ name: 'Text Diff', path: '/text/diff', description: 'Compare sorted vs unsorted' },
			{ name: 'Find & Replace', path: '/text/find-replace', description: 'Pattern-based line filtering' },
	...
	'diff': {
		...
		examples: [
			{ label: 'Config file change', code: 'Before: port=8080\nAfter: port=3000\n→ Shows "8080" deleted, "3000" added', isValid: true },
			{ label: 'Code refactoring', code: 'Before: function foo() {...}\nAfter: const foo = () => {...}\n→ Highlights syntax change', isValid: true },
			{ label: 'Documentation update', code: 'Before: "Version 1.0 released"\nAfter: "Version 2.0 released"\n→ Shows version number change', isValid: true }
		],
		faqs: [
			{ question: 'What\'s the difference between side-by-side and inline diff?', answer: '<p>Side-by-side shows both versions next to each other, making it easy to compare. Inline shows changes in a single column with +/- markers, like Git diff output.</p>' },
			{ question: 'Should I ignore whitespace when comparing code?', answer: '<p>Yes, if you only care about actual code changes. Ignore whitespace to hide indentation differences caused by auto-formatting tools.</p>' },
			{ question: 'How does this compare to Git diff?', answer: '<p>This uses the same Myers diff algorithm as Git. The difference is this runs in your browser without needing Git installed, and has a more visual interface.</p>' },
			{ question: 'Can I diff JSON or XML files?', answer: '<p>Yes, but format them first! Use a JSON/XML formatter to ensure consistent indentation, then diff the formatted versions for cleaner comparison.</p>' },
			{ question: 'What are context lines?', answer: '<p>Context lines are unchanged lines shown around changes to give you context. For example, 3 context lines shows 3 unchanged lines before and after each change.</p>' }
		],
		relatedTools: [
			{ name: 'String Compare', path: '/text/string-compare', description: 'Character-level comparison' },
			{ name: 'Find & Replace', path: '/text/find-replace', description: 'Find differences with regex' },
	...
	'find-replace': {
		...
		examples: [
			{ label: 'Replace URLs', code: 'Find: http://example.com\nReplace: https://example.com\n→ Updates all HTTP to HTTPS', isValid: true },
			{ label: 'Format phone numbers', code: 'Find: (\\d{3})(\\d{3})(\\d{4})\nReplace: ($1) $2-$3\n→ "1234567890" becomes "(123) 456-7890"', isValid: true },
			{ label: 'Remove extra spaces', code: 'Find: \\s+\nReplace: " " (single space)\n→ Collapses multiple spaces', isValid: true }
		],
		faqs: [
			{ question: 'What is regex and why should I use it?', answer: '<p>Regex (regular expressions) is a powerful pattern-matching language. Instead of finding exact text, you can find patterns like "any email address" or "all phone numbers". It\'s essential for advanced text processing.</p>' },
			{ question: 'How do I reference captured groups in replacement?', answer: '<p>Use $1, $2, etc. For example, if you capture email with <code>(\\w+)@(\\w+\\.com)</code>, you can reverse it with <code>$2 - $1</code> in the replacement field.</p>' },
			{ question: 'What does "global" replace mean?', answer: '<p>Global replaces all occurrences in the text. Without global, only the first match on each line is replaced. Always use global unless you specifically want to replace only the first match.</p>' },
			{ question: 'Can I preview changes before applying?', answer: '<p>Yes! The tool shows a preview of what will be changed, with matches highlighted. This prevents accidental destructive replacements.</p>' },
			{ question: 'How do I escape special characters in regex?', answer: '<p>Use backslash (\\) before special characters like . * + ? [ ] ( ) { } ^ $ |. For example, to match a literal period, use <code>\\.</code></p>' }
		],
		relatedTools: [
			{ name: 'String Compare', path: '/text/string-compare', description: 'Verify replacements worked' },
	...
	'statistics': {
		...
		examples: [
			{ label: 'Tweet length check', code: 'Input: "Just launched my new website!"\nCharacters: 30 (under 280 limit) ✓', isValid: true },
			{ label: 'Blog post analysis', code: 'Input: 1,847 words\nReading time: ~7 minutes\nSentences: 94', isValid: true },
			{ label: 'Essay word count', code: 'Input: 498 words\nTarget: 500 words\nNeed: 2 more words', isValid: true }
		],
		faqs: [
			{ question: 'How is reading time calculated?', answer: '<p>Reading time assumes an average reading speed of 200-250 words per minute for adults. The tool uses 238 WPM (based on research), though individual speeds vary from 150-400 WPM.</p>' },
			{ question: 'Does character count include spaces?', answer: '<p>Yes! Character count includes spaces, punctuation, and special characters. Some platforms (like Twitter) count spaces, while others don\'t—this tool shows both.</p>' },
			{ question: 'What counts as a word?', answer: '<p>A word is any sequence of characters separated by spaces. Contractions like "don\'t" count as one word. Hyphenated words like "well-being" also count as one word.</p>' },
			{ question: 'Can I use this for SEO optimization?', answer: '<p>Yes! Content length is an SEO factor. Most SEO experts recommend 1,500-2,500 words for blog posts to rank well. Use this to check if you\'ve hit your target length.</p>' },
			{ question: 'What\'s the difference between unique words and total words?', answer: '<p>Total words counts every word, including repeats. Unique words counts each distinct word only once. High uniqu ratio indicates diverse vocabulary.</p>' }
		],
		relatedTools: [
			{ name: 'Text Diff', path: '/text/diff', description: 'Compare word counts between versions' },
			...
			{ name: 'Remove Duplicates', path: '/text/remove-duplicates', description: 'Remove duplicate words' }
		],
		tips: [
	...
	'anagram': {
		...
		examples: [
			{ label: 'Simple anagram', code: 'Input: "listen"\nAnagrams: silent, enlist, inlets, tinsel', isValid: true },
			{ label: 'Name anagram', code: 'Input: "William Shakespeare"\nAnagram: "I am a weakish speller"', isValid: true },
			{ label: 'Phrase anagram', code: 'Input: "the eyes"\nAnagram: "they see"', isValid: true }
		],
		faqs: [
			{ question: 'What makes a valid anagram?', answer: '<p>A valid anagram uses all the letters from the original word exactly once—no more, no fewer. "Cat" cannot be an anagram of "bat" because the letters differ.</p>' },
			{ question: 'Does the anagram finder include proper nouns?', answer: '<p>By default, the tool focuses on common English words. Proper nouns (names, places) are generally excluded unless they\'re also common words (like "bill" or "rose").</p>' },
			{ question: 'Can I find anagrams of phrases?', answer: '<p>Yes! For phrases, the tool ignores spaces and punctuation, treating "the eyes" and "they see" as anagrams because they have the same letters.</p>' },
			{ question: 'How many anagrams exist for a typical word?', answer: '<p>It varies widely. Common short words might have 5-20 anagrams. Longer words often have fewer because the letter combination becomes more unique.</p>' },
			{ question: 'Can this help with Wordle or Scrabble?', answer: '<p>Absolutely! Enter your available letters, and the tool finds all valid words you can make. For Wordle, use it to find words with specific letter patterns.</p>' }
		],
		relatedTools: [
			{ name: 'String Compare', path: '/text/string-compare', description: 'Compare word similarities' },
	...
	'remove-duplicates': {
		...
		examples: [
			{ label: 'Email list cleanup', code: 'Input:\nuser@example.com\ntest@test.com\nuser@example.com\n→ Removes second "user@example.com"', isValid: true },
			{ label: 'Repeated words', code: 'Input: "The the quick brown brown fox"\n→ Output: "The quick brown fox"', isValid: true },
			{ label: 'Case-insensitive', code: 'Input:\nApple\napple\nBanana\n→ Keeps "Apple", removes "apple"', isValid: true }
		],
		faqs: [
			{ question: 'Does removing duplicates change the order of items?', answer: '<p>No! The tool preserves the original order. The first occurrence of each unique item stays in its original position—only subsequent duplicates are removed.</p>' },
			{ question: 'What\'s the difference between line and word deduplication?', answer: '<p>Line deduplication treats each line as a unit. Word deduplication works within lines, removing repeated words while keeping the line structure.</p>' },
			{ question: 'Should I use case-sensitive or case-insensitive mode?', answer: '<p>Use case-insensitive for email lists, names, or URLs where "Example.com" and "example.com" should be considered the same. Use case-sensitive for code or where capitalization matters.</p>' },
			{ question: 'Does the tool trim whitespace before comparing?', answer: '<p>Yes, by default! Leading/trailing spaces are removed before comparison, so "  apple" and "apple  " are treated as duplicates. You can disable this if needed.</p>' },
			{ question: 'How can I see how many duplicates were removed?', answer: '<p>The tool shows statistics: original line count, unique line count, and number of duplicates removed. This helps verify the cleanup worked correctly.</p>' }
		],
		relatedTools: [
			{ name: 'Line Tools', path: '/text/line-tools', description: 'Sort and manipulate lines' },
			...
			{ name: 'Find & Replace', path: '/text/find-replace', description: 'Find duplicates with regex' }
		],
		tips: [
	...
	'lorem-ipsum': {
		...
		examples: [
			{ label: 'Single paragraph', code: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt...', isValid: true },
			{ label: 'Three short sentences', code: 'Lorem ipsum dolor sit amet. Consectetur adipiscing elit. Sed do eiusmod tempor.', isValid: true },
			{ label: '50 words', code: 'Lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor incididunt...', isValid: true }
		],
		faqs: [
			{ question: 'What does "Lorem Ipsum" mean?', answer: '<p>It doesn\'t mean anything! The text is scrambled Latin from Cicero\'s writings. The words are broken and rearranged so readers focus on layout, not content.</p>' },
			{ question: 'Why not just use "test test test"?', answer: '<p>Real text has varying word lengths, letter frequencies, and punctuation. Lorem Ipsum mimics this natural distribution, giving more realistic layout testing than repeated words.</p>' },
			{ question: 'Can I generate text with HTML tags?', answer: '<p>Yes! The tool can wrap paragraphs in <code>&lt;p&gt;</code> tags, making it easy to paste directly into HTML code without manual formatting.</p>' },
			{ question: 'Is Lorem Ipsum still relevant in modern design?', answer: '<p>Absolutely! It\'s an industry standard in web design, graphic design, and publishing. Clients and designers immediately recognize it as placeholder text.</p>' },
			{ question: 'What\'s the difference between paragraphs, sentences, and words?', answer: '<p>Paragraphs generate full blocks of text separated by line breaks. Sentences generate complete sentences with periods. Words generate just words separated by spaces, no punctuation.</p>' }
		],
		relatedTools: [
			{ name: 'Blabber Generator', path: '/text/blabber', description: 'Generate random readable text' },
			...
			{ name: 'Find & Replace', path: '/text/find-replace', description: 'Customize generated text' }
		]
	},
	'blabber': {
		features: [
		...
		examples: [
			{ label: 'Generated paragraph', code: 'Mikalora befindo ralesta. Kolpa mindara selko befindo ralesta polikarn...', isValid: true },
			{ label: 'Short blabber', code: 'Beka milo. Randa kolf pesi. Mika belo ranta.', isValid: true },
			{ label: 'Long form text', code: 'Mikolandara befindorel kolikorestana polikestran mirandoleskal...', isValid: true }
		],
		faqs: [
			{ question: 'How is this different from Lorem Ipsum?', answer: '<p>Lorem Ipsum is scrambled Latin text. Blabber generates completely random pronounceable words that look like a made-up language. Use Blabber when you want unique, non-Latin placeholder text.</p>' },
			{ question: 'Is the generated text truly random?', answer: '<p>Yes! Each time you generate, you get completely different words. The algorithm creates new syllable combinations every time, so no two generations are identical.</p>' },
			{ question: 'Can I use this for multilingual testing?', answer: '<p>Yes! Blabber text works well for testing layouts with "foreign-looking" text without using real languages that might have cultural or translation issues.</p>' },
			{ question: 'Why use gibberish instead of real placeholder text?', answer: '<p>Real text can distract stakeholders or users during reviews. Gibberish makes it obvious that content is temporary while still showing realistic text flow.</p>' },
			{ question: 'Is the generated text repeatable?', answer: '<p>No, it\'s random each time. If you need the same placeholder text across multiple elements, generate once and copy/paste or use Lorem Ipsum which has standard paragraphs.</p>' }
		],
		relatedTools: [
			{ name: 'Lorem Ipsum', path: '/text/lorem-ipsum', description: 'Generate classic Lorem Ipsum' },
